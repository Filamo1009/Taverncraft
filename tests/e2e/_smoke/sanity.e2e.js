...
//
// 1. Spawns a Taverncraft server on its own port + cloned dataRoot.
...
//    chat-completion request from Taverncraft's server.
//
...
test('sanity: dedicated server + mock backend + first-turn happy path', async ({ page }) => {
    ...
    const initialChatLen = await page.evaluate(() => window.Taverncraft.getContext().chat?.length || 0);
