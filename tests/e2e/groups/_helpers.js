 ...
 * @returns {string} avatarFile (== character avatar id in Taverncraft)
 */
...
export async function sendUserAndAwaitGroupTurn(page, text, { timeoutMs = 120_000 } = {}) {
    const lengthBefore = await page.evaluate(() => window.Taverncraft.getContext().chat?.length || 0);
    const wrapperDonePromise = page.evaluate((to) => new Promise((resolve, reject) => {
        ...
        const handler = (payload) => {
            clearTimeout(t);
            try { ctx.eventSource.removeListener(ctx.eventTypes.GROUP_WRAPPER_FINISHED, handler); } catch { /* listener may already be removed */ }
            resolve(payload);
        };
