export class WebLlmVectorProvider {
     ...
     */
    #checkWebLlm() {
        if (!Object.hasOwn(Taverncraft, 'llm')) {
            throw new Error('WebLLM is not available', { cause: 'webllm-not-available' });
        }

        if (typeof Taverncraft.llm.generateEmbedding !== 'function') {
    ...
    #initEngine(modelId) {
        this.#checkWebLlm();
        if (!this.#engine) {
            this.#engine = Taverncraft.llm.getEngine();
        }
    ...
    getModels() {
        this.#checkWebLlm();
        return Taverncraft.llm.getEmbeddingModels();
    }
