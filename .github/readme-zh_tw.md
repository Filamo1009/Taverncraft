...
Taverncraft 是 SillyTavern 的一個 fork，重點在更優雅的 API 設計與更強的擴充能力。

- 增量聊天儲存 API（`append` / `patch` / `meta/patch` / `state/patch`）。
- 後端託管的生成任務恢復能力（前端斷線後可恢復）。
- 內建 `Orchestrator` 與 `Memory` 外掛。
- API 遷移文件：[docs/luker-api-migration.md](https://github.com/Filamo1009/Taverncraft/blob/release/docs/luker-api-migration.md)
