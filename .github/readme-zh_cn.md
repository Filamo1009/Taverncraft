...
Taverncraft 是 SillyTavern 的一个 fork，重点在于更优雅的 API 设计与更强的扩展能力。

- 增量聊天存储接口（`append` / `patch` / `meta/patch` / `state/patch`）。
- 后端托管的生成任务恢复能力（前端断连后可恢复）。
- 内置 `Orchestrator` 与 `Memory` 插件。
- API 迁移文档：[docs/luker-api-migration.md](https://github.com/Filamo1009/Taverncraft/blob/release/docs/luker-api-migration.md)
