...
## Taverncraft 포크 변경 사항

Taverncraft는 SillyTavern 포크이며, 더 깔끔한 API 설계와 확장성 강화에 중점을 둡니다.

- 증분 저장 API (`append` / `patch` / `meta/patch` / `state/patch`).
- 재접속 시 복구 가능한 백엔드 생성 작업 관리.
- 내장 플러그인: `Orchestrator`, `Memory`.
- API 마이그레이션 문서: [docs/luker-api-migration.md](https://github.com/Filamo1009/Taverncraft/blob/release/docs/luker-api-migration.md)
