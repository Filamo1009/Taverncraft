...
export async function getVersion() {
    ...
    const agent = `Taverncraft:${pkgVersion}:Cohee#1207`;
    const compatAgent = `Taverncraft:${stCompatVersion}:Cohee#1207`;
        ...
        if (commandExistsSync('git')) {
            ...
            try {
                ...
                const remoteUrl = String(
                    process.env.TAVERNCRAFT_UPDATE_REMOTE
                    || process.env.LUKER_UPDATE_REMOTE
                    || 'https://github.com/Filamo1009/Taverncraft.git',
                ).trim();
                if (remoteUrl) {
                    try {
