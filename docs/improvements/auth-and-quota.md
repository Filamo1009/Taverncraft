...
Taverncraft has a built-in multi-user authentication and quota management system, suitable for team-shared instances or publicly deployed scenarios. Administrators can comprehensively manage user access and resource consumption through OAuth login, storage quotas, and the logging system.
...
Taverncraft supports user authentication via GitHub OAuth. When users click login, they are redirected to the GitHub authorization page. After authorization is complete, a local user account is automatically created or associated.
...
Taverncraft also supports Discord OAuth login, with more fine-grained access control — it can verify whether a user is a member of a specified Discord server and whether they have specific roles.
...
::: tip
If you don't need OAuth login, there's no need to configure the `oauth` section. Taverncraft still supports SillyTavern's original user authentication methods. OAuth configuration is recommended to be done through the frontend admin panel.
:::
...
Taverncraft implements a server-side log capture system for administrators to remotely view server operational status.

Taverncraft's logging system covers both backend and frontend, helping administrators troubleshoot issues remotely. See [Logging System](/features/logging) for details.
