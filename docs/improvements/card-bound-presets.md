...
A character card may embed any number of chat completion presets, each with its own name and body. When you load a character with embedded presets, Taverncraft:
...
Open the character-management dropdown next to the character portrait and choose **Bind Current Chat Completion Preset**. Taverncraft adds the currently selected preset to the card's embedded set and marks it as the card's default.

- If the preset name is not yet on the card, it is added as a new slot.
- If a slot with the same name already exists, Taverncraft asks whether to overwrite the existing card copy with your current settings.

Binding is blocked when the currently selected preset is *itself* a card-bound option — there is nothing to promote in that case, and Taverncraft surfaces an info notice instead of silently no-oping.
...
- **Set as default** — pick which slot Taverncraft auto-applies when the character is loaded.
...
**Save To Character Override** in the orchestrator drawer inspects the referenced preset names before persisting. If any agent references a preset that is not yet on the card, Taverncraft prompts:
...
- If you have not chosen a persona, Taverncraft switches to the card's recommendation.
- If you already have a persona, Taverncraft warns that it differs from the card's recommendation, so you can choose whether to align or keep your own.
