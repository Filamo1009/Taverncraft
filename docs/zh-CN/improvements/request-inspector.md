...
请求检查器（Request Inspector）是 Taverncraft 的核心后端模块之一，用于追踪每个 AI 生成请求从发起到完成的完整生命周期，并记录详细的 Token 用量数据。它是生成诊断能力的基础设施。

## 问题背景

在 SillyTavern 中，AI 生成请求发出后，后端不会系统性地记录请求的 Token 消耗。用户无法得知每次生成实际花费了多少 Token，管理员也无法追踪多用户场景下的资源使用情况。

Taverncraft 实现了一套完整的请求生命周期追踪系统，覆盖文本生成和图像生成两类请求。
