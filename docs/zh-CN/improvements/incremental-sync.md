...
增量同步是 Taverncraft 对 SillyTavern 数据传输架构的根本性改进，用增量 patch 替代全量覆盖，大幅降低带宽消耗并消除并发写入冲突。
...
Taverncraft 新增了三个增量端点，覆盖聊天数据的不同修改场景：
...
除了聊天数据，Taverncraft 的设置数据（用户偏好、扩展配置、预设参数等）同样支持增量 patch 保存。设置变更通过深度合并应用到服务端的设置文件中，同样带有 integrity hash 冲突检测，避免并发修改导致设置丢失。
...
::: tip 性能收益
对于一个包含 500 条消息的聊天记录，编辑一条消息时：SillyTavern 需要传输约 2-5 MB 的全量数据，而 Taverncraft 只需传输约 1-2 KB 的 patch 数据。在移动网络或高延迟环境下，差异尤为明显。
:::
