...
Taverncraft 重新设计了数据持久化架构，将数据变更的保存职责从前端转移到后端，实现实时持久化，从根本上消除因浏览器崩溃、网络中断或意外关闭导致的数据丢失风险。
...
在 AI 生成场景中，Taverncraft 的统一生成层实现了 Generation Acknowledge 机制。当后端完成一次生成并将结果持久化后，会在响应中确认生成结果已被服务端安全存储。
...
::: tip 与传统模式的对比
在 SillyTavern 中，AI 生成的结果先到达前端，由前端决定何时保存。如果前端在保存前崩溃，生成的内容就会丢失。Taverncraft 的 Generation Acknowledge 将保存时机提前到了后端响应之前，从根本上消除了这个窗口期。
:::

## 序列化聊天写入 {#序列化聊天写入}

为了防止并发写入导致文件损坏，Taverncraft 在前端（通过 `runSerializedChatWrite`）对聊天写入任务做串行化，同时由后端对每次写入执行 integrity 校验。
