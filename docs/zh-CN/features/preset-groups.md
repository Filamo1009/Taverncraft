...
预设分组是 Taverncraft 为预设选择器引入的分组展示功能。当你积累了大量预设后，扁平的下拉列表会变得难以管理。预设分组允许你将预设归入命名分组，在下拉选择器中以分组 header + 成员的层级结构展示。
...
Taverncraft 实现了自定义的 Select2 适配器，用于在下拉选择器中渲染分组结构和操作按钮：
