...
::: tip 适用场景
本指南面向「Termux 上的 SillyTavern → Taverncraft APK」。PC / Linux / Docker 上的 SillyTavern 迁到 Taverncraft 服务端、或者 Termux 之间互迁，请参考 [从 SillyTavern 迁移](/zh-CN/guide/migration)。
:::
...
::: tip ZIP 内部前缀宽容
Taverncraft 的导入逻辑会自动识别 ZIP 里 `characters/`、`chats/`、`worlds/` 这类子目录，不管它前面套着 `data/default-user/...`、`default-user/...`、还是直接 `characters/...`，都能正确落到对应位置。
:::

### 2. 安装 Taverncraft APK 并触发首次启动向导

从 [GitHub Releases](https://github.com/Filamo1009/Taverncraft/releases/latest) 下载安装最新 APK。

**首次打开** Taverncraft 会弹出标题为 **「欢迎来到 Taverncraft！」** 的引导窗口。窗口顶部可以选界面语言（Language，默认英文，建议先切到「简体中文」，下面的按钮就会变中文），中间是一块 **「从 SillyTavern 迁移」**，列出三个并排按钮：
...
### 3. 完成引导后进入 Taverncraft

引导窗口要求填一个用户名（用作默认人设名），完成后 Taverncraft 会进入主界面。这时角色卡列表、聊天记录、世界书、预设、API 密钥都应已经在位。

## 错过了首次启动向导怎么办

如果你已经关掉了引导窗口（或者 Taverncraft 已经用了一段时间想再次导入），可以从用户管理面板再走一次相同的导入：
...
## 局域网迁移（双方都已是 Taverncraft 时）

如果你的源端已经是 Taverncraft（另一台 Taverncraft APK、或者 Termux 里跑的服务端 Taverncraft），目标端也是 Taverncraft APK，可以走更轻松的局域网迁移，完全不用打 ZIP：

1. 源端「备份与恢复」面板里点 **「创建迁移链接」**，生成一次性链接
2. 目标端把链接粘进 **「从链接迁移」** 框，点导入
3. 同局域网内自动传输 + 恢复

链接是一次性的、很快过期。SillyTavern 原版没有这个 UI，所以从原版 SillyTavern 来的迁移走不通——先按上面的 ZIP 流程到 Taverncraft，之后的迁移就能用「局域网迁移」了。
...
- 你需要保留 Taverncraft 导入逻辑识别不了的额外文件
- 你想精确替换某个特定文件而不是整体导入

Taverncraft APK 的数据目录在 `/storage/emulated/0/Android/data/com.luker.app/files/luker-data/`，结构与 SillyTavern 的 `data/` 目录一致。Android 11+ 对这个路径加了访问限制，普通文件管理器多半进不去，需要授权或换工具。
...
### 2. 首次启动 Taverncraft APK 让它创建数据目录

打开一次 APK 让 `luker-data/` 目录生成出来。这一步必须做，否则下面要复制的目标目录还不存在。

### 3. 强制停止 Taverncraft

进入系统设置 → 应用 → Taverncraft → **强制停止**，确保 Taverncraft 后台服务完全退出。迁移期间不要让 Taverncraft 在后台运行，否则它可能在你复制文件的同时改写数据。

### 4. 把数据搬进 Taverncraft 数据目录
...
Taverncraft APK 自己在 `luker-data/` 下维护一个 `_runtime-persist/` 目录，存放运行时持久化产物。**不要覆盖也不要删除它**，从原版 SillyTavern 也不会带来这个目录。
:::

### 5. 重启 Taverncraft，验证数据

重新打开 Taverncraft APK。等待加载完成后角色卡列表、聊天记录、世界书、预设、API 连接都应自动恢复。
...
| 内容 | Termux 原路径 | Taverncraft APK 目标路径 |
| --- | --- | --- |
...
引导只在全新数据目录上首次打开时弹出。如果 Taverncraft 之前已经被打开过，从用户管理面板的「备份与恢复」做同样的导入即可——点「全选」，选 ZIP，选「增量更新」，然后点「恢复备份」。
...
**重启 Taverncraft 后角色卡列表是空的（手动复制流程）**

检查复制层级：`luker-data/` 下应直接出现 `default-user/`（或其他用户名目录），而不是嵌套的 `luker-data/data/default-user/`。如果出现了重复的 `data/` 一层，把里面的内容上移一级。

**Taverncraft 启动后白屏 / 报错（手动复制流程）**

可能是 `_runtime-persist/` 被覆盖或破坏。可以尝试：① 删除 `_runtime-persist/` 让 Taverncraft 自动重建；② 仍有问题时进入「应用信息 → 存储 → 清除数据」并重新走一遍迁移流程。
...
迁移后 Taverncraft 不会修改原 SillyTavern 数据格式，只在 `data/` 中追加自己的状态文件（如 `.luker-state.<chat_id>.json` 等）。如果你之后又想回到 Termux 上的 SillyTavern，把 `luker-data/` 的内容拷回去即可，SillyTavern 会忽略它无法识别的状态文件。
...
- [快速开始](/zh-CN/guide/getting-started) — Taverncraft 安装总览
