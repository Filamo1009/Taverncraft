...
公告功能让管理员向同一台 Taverncraft 实例上的所有本地账号广播一条 Markdown 消息。典型场景：

- 告知其他账号「升级后某个 UI 按钮换位置/改名了」。
- 通知用户计划内的停机或维护。
- 事件后说明（「我刚从备份恢复了 staging CardApp，你们的书签可能失效」）。

如果你独自使用 Taverncraft、没有其他本地账号，这个功能对你没意义。
...
用户打开 Taverncraft 时若有未读 `严重` 公告，会弹出一个弹窗列出所有未读的严重公告。点**全部标为已读**关闭弹窗并一次性把所有列出的公告标为已读。除非再发布新的 `严重` 公告，用户下次加载时不会再被打扰。
