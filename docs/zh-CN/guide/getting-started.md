...
本指南将帮助你在几分钟内启动并运行 Taverncraft。

## 前置条件

| 依赖 | 要求 |
| --- | --- |
| **Node.js** | >= 20.18.1（仅 Git Clone 方式需要；以 `package.json` 声明为准） |
...
::: tip 其他运行时
Taverncraft 也支持通过 Deno 或 Bun 启动：
```bash
# Deno
npm run start:deno
...
适合服务器部署或希望开箱即用的用户。Taverncraft 提供了预构建的 Docker 镜像。
...
Taverncraft 提供安卓 APP，你可以直接在手机上运行 Taverncraft，无需依赖云服务器或 Termux。
...
首次进入 Taverncraft 后，你需要配置至少一个 LLM API 才能开始对话：
...
Taverncraft 支持当前兼容测试覆盖的标准 SillyTavern 角色卡、世界书、Persona、预设和 JSONL 聊天格式。迁移前请备份源数据，按照迁移指南操作，并在完成后打开一段已知聊天进行核对。Taverncraft 专属状态保存在扩展数据中，不理解它的客户端可能忽略这些内容；任意第三方扩展不保证可直接迁移。

::: warning 备份提醒
迁移或降级前必须备份。除非相应路径已经验证，不要假设其他客户端一定会保留未知扩展字段。
:::
