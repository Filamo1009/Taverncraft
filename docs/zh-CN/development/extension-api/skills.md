...
`context.skills.*` 是用于安装、读取、编辑、打包 Skills 的 JavaScript 接口。扩展通过 `Taverncraft.getContext()` 拿到它；CardApp 在自己的 `ctx.skills` 上拿到同样的形状。
...
CardApp ctx 接口是底层同一组函数的薄包装 —— 调用签名与返回形状相同（遵循 Taverncraft 的 API 对等约定）。
