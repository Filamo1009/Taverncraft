...
Taverncraft 通过 `macros` 命名空间提供宏系统，同时为向后兼容保留了 `MacrosParser`。<span v-pre>`{{user}}`</span>、<span v-pre>`{{char}}`</span>、<span v-pre>`{{lastMessage}}`</span>、<span v-pre>`{{getvar::name}}`</span> 等内置宏由核心注册；插件可以通过 `macros.register()` 添加自己的宏。
