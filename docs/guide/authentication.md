...
Taverncraft provides multi-layered access control and security mechanisms, from simple password protection to OAuth social login and storage quota management, covering deployment scenarios ranging from personal use to team sharing.

## Basic Auth Mode

The simplest form of access protection. When enabled, accessing Taverncraft requires a username and password:
...
Suitable for single-user scenarios, such as when you've deployed Taverncraft on a server and want to prevent others from accessing it.
...
::: tip
This command must be run from the Taverncraft project root directory. The server does not need to be running.
:::

### Web-Based Password Recovery

In addition to the command-line tool, Taverncraft supports initiating password recovery from the web interface. When a user clicks "Forgot Password" on the login page, the system generates a recovery code and **prints it to the server console**. The administrator provides the recovery code to the user, who can then complete the password reset on the web page.
...
When enabled, the login page won't display obvious Taverncraft branding — useful for low-profile deployments.
...
::: warning
Self-service registration is intended for trusted networks or invite-gated deployments. If you expose Taverncraft to the public internet, prefer OAuth or admin-created accounts so you can vet new users.
:::

## GitHub OAuth Login

Taverncraft supports logging in with a GitHub account. Users are redirected to GitHub's authorization page, and upon completion, a local user account is automatically created or linked.
...
Taverncraft also supports Discord OAuth login with finer-grained access control — it can verify whether a user is a member of a specific Discord server and whether they hold specific roles.
...
This is particularly useful for community-shared Taverncraft instances — only specific members of your Discord server can log in and use it.

::: tip
If you don't need OAuth login, there's no need to enable it in the Admin Panel. Taverncraft still supports Basic Auth and multi-user account authentication as described above.
:::
...
Taverncraft enables IP whitelist mode by default, and `listen` defaults to `false` (only listening on the loopback address), so LAN devices cannot connect directly. To allow LAN access, you need to do two things:

1. **Enable listening** — Set `listen` to `true` so that Taverncraft listens on all network interfaces:
...
Taverncraft also supports a host whitelist (`hostWhitelist`) that can restrict allowed host addresses and supports automatic LAN host scanning.

## Security Best Practices

If you need to expose Taverncraft to the public internet, make sure to implement the following security measures:
...
5. **Keep Updated** — Update Taverncraft regularly to get the latest security fixes

::: warning
Never expose Taverncraft to the public internet without any authentication mechanism. Even for internal network deployments, it's recommended to enable at least Basic Auth protection.
:::
