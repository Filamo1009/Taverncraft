...
CardApp is Taverncraft's unique in-card application system. It allows character cards to define small applications (including HTML, JavaScript, and styles) within `data.extensions.card_app`, which are loaded and rendered in the chat interface, giving character cards interactive and dynamic capabilities.
...
2. **Run** — The app interacts with Taverncraft through the `ctx` context object, responding to chat events and updating its own state
