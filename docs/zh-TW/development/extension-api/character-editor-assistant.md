...
`character-editor-assistant`（CEA）擴充透過 Taverncraft 的 extension 註冊表發布三個 helper，讓其他外掛擁有的迭代工作台能複用 CEA 的 helper-tool 面，而不必跨外掛邊界 import。這正是編排器 iter-studio 彈窗和記憶圖 schema iter-studio 彈窗共同消費的介面。
...
回傳的陣列恆定包含四個 helper（lorebook 讀、lorebook 寫、simulate、world-book-list），當 `globalThis.Taverncraft.searchTools` 接好時會額外多一個 web 搜尋 helper。
