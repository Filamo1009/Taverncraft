...
`context.skills.*` 是用於安裝、讀取、編輯、打包 Skills 的 JavaScript 介面。擴充透過 `Taverncraft.getContext()` 拿到它；CardApp 在自己的 `ctx.skills` 上拿到同樣的形狀。
...
CardApp ctx 介面是底層同一組函式的薄包裝 —— 呼叫簽名與返回形狀相同（遵循 Taverncraft 的 API 對等約定）。
