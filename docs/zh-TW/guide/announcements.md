...
公告功能讓管理員向同一台 Taverncraft 實例上的所有本機帳號廣播一條 Markdown 訊息。典型場景：

- 告知其他帳號「升級後某個 UI 按鈕換位置/改名了」。
- 通知使用者計畫內的停機或維護。
- 事件後說明（「我剛從備份還原了 staging CardApp，你們的書籤可能失效」）。

如果你自己使用 Taverncraft、沒有其他本機帳號，這個功能對你沒意義。
...
使用者打開 Taverncraft 時若有未讀 `嚴重` 公告，會跳出一個彈窗列出所有未讀的嚴重公告。點**全部標為已讀**關閉彈窗並一次把所有列出的公告標為已讀。除非再發布新的 `嚴重` 公告，使用者下次載入時不會再被打擾。
