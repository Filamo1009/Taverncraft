...
Taverncraft 提供了多層次的存取控制和安全機制，從簡單的密碼保護到 OAuth 社交登入，再到儲存配額管理，滿足從個人使用到團隊共享的各種部署場景。

## Basic Auth 模式

最簡單的存取保護方式。啟用後，存取 Taverncraft 時需要輸入使用者名稱和密碼：
...
適用於單一使用者場景，例如你在伺服器上部署了 Taverncraft 並希望防止他人存取。
...
::: tip
此命令需要在 Taverncraft 專案根目錄下執行，服務無需處於執行狀態。
:::

### 網頁端密碼復原

除了命令列工具，Taverncraft 還支援透過網頁端發起密碼復原流程。使用者在登入頁面點擊「忘記密碼」後，系統會產生一個復原碼並**輸出在伺服器端主控台**中。管理員將復原碼告知使用者，使用者即可在網頁上完成密碼重設。
...
啟用後，登入頁面不會顯示明顯的 Taverncraft 標識，適用於需要低調部署的場景。
...
::: warning
自助註冊適用於可信任網路或邀請制部署。若 Taverncraft 暴露在公網，建議改用 OAuth 或由管理員手動建立帳號，以便核驗新使用者身分。
:::

## GitHub OAuth 登入

Taverncraft 支援透過 GitHub 帳號登入。使用者點擊登入後會被重新導向到 GitHub 授權頁面，授權完成後自動建立或關聯本機使用者帳號。
...
Taverncraft 同樣支援 Discord OAuth 登入，並提供更細緻的存取控制——可以校驗使用者是否為指定 Discord 伺服器的成員，以及是否擁有特定身分組（Role）。
...
這對於社群共享的 Taverncraft 實例特別有用——只有你 Discord 伺服器中的特定成員才能登入使用。

::: tip
如果不需要 OAuth 登入，無需在管理面板中啟用 OAuth。Taverncraft 仍然支援上述的 Basic Auth 和多使用者帳號認證方式。
:::
...
Taverncraft 預設啟用 IP 白名單模式，且 `listen` 預設為 `false`（僅監聽本機回環地址），區域網路無法直接存取。如需區域網路存取，需要同時做兩件事：

1. **開啟監聽** — 將 `listen` 設為 `true`，使 Taverncraft 監聽所有網路介面：
...
此外，Taverncraft 還支援主機白名單（`hostWhitelist`），可以限制允許存取的主機地址，並支援自動掃描區域網路主機。

## 安全最佳實務

如果你需要將 Taverncraft 暴露到公網，請務必做好以下安全防護：
...
5. **定期更新** — 及時更新 Taverncraft 以取得最新的安全修復

::: warning
不要在沒有任何認證機制的情況下將 Taverncraft 暴露到公網。即使是內網部署，也建議至少啟用 Basic Auth 保護。
:::
