# 從 Termux 原版酒館遷移到 Taverncraft APK

如果你之前在 Termux 裡跑原版 SillyTavern，想換用 Taverncraft 的 Android APK，這篇指南說明如何把原有資料搬到 APK 中。目前相容性測試涵蓋標準角色卡、世界書、Persona、預設與 JSONL 聊天路徑；APK 首次啟動時會彈出匯入精靈，正常情況下不需要碰 `Android/data` 私有目錄。遷移前仍需備份並核對第三方擴充。

遷移完成後，你的角色卡、聊天紀錄、世界書、預設、使用者人設、擴充功能設定都會照常載入。

::: tip 適用場景
本指南面向「Termux 上的 SillyTavern → Taverncraft APK」。PC / Linux / Docker 上的 SillyTavern 遷到 Taverncraft 服務端、或者 Termux 之間互遷，請參考 [從 SillyTavern 遷移](/zh-TW/guide/migration)。
:::
...
::: tip ZIP 內部前綴寬容
Taverncraft 的匯入邏輯會自動辨識 ZIP 裡 `characters/`、`chats/`、`worlds/` 這類子目錄，不管它前面套著 `data/default-user/...`、`default-user/...`、還是直接 `characters/...`，都能正確落到對應位置。
:::

### 2. 安裝 Taverncraft APK 並觸發首次啟動精靈

從 [GitHub Releases](https://github.com/Filamo1009/Taverncraft/releases/latest) 下載安裝最新 APK。

**首次開啟** Taverncraft 會彈出標題為 **「歡迎來到 Taverncraft！」** 的引導視窗。視窗頂部可以選介面語言（Language，預設英文，建議先切到「繁體中文」，下面的按鈕就會變中文），中間是一塊 **「從 SillyTavern 遷移」**，列出三個並排按鈕：
...
### 3. 完成精靈後進入 Taverncraft

引導視窗要求填一個使用者名稱（用作預設人設名），完成後 Taverncraft 會進入主介面。這時角色卡列表、聊天紀錄、世界書、預設、API 金鑰都應已經就位。

## 錯過了首次啟動精靈怎麼辦

如果你已經關掉了引導視窗（或者 Taverncraft 已經用了一段時間想再次匯入），可以從使用者管理面板再走一次相同的匯入：
...
## 區域網遷移（雙方都已是 Taverncraft 時）

如果你的源端已經是 Taverncraft（另一台 Taverncraft APK、或者 Termux 裡跑的服務端 Taverncraft），目標端也是 Taverncraft APK，可以走更輕鬆的區域網遷移，完全不用打 ZIP：

1. 源端「備份與還原」面板裡點 **「建立遷移連結」**，產生一次性連結
2. 目標端把連結貼進 **「從連結遷移」** 框，點匯入
3. 同區域網內自動傳輸 + 還原

連結是一次性的、很快過期。SillyTavern 原版沒有這個 UI，所以從原版 SillyTavern 來的遷移走不通——先按上面的 ZIP 流程到 Taverncraft，之後的遷移就能用「區域網遷移」了。
...
- 你需要保留 Taverncraft 匯入邏輯辨識不了的額外檔案
- 你想精確替換某個特定檔案而不是整體匯入

Taverncraft APK 的資料目錄在 `/storage/emulated/0/Android/data/com.luker.app/files/luker-data/`，結構與 SillyTavern 的 `data/` 目錄一致。Android 11+ 對這個路徑加了存取限制，普通檔案管理員多半進不去，需要授權或換工具。
...
### 2. 首次啟動 Taverncraft APK 讓它建立資料目錄

開啟一次 APK 讓 `luker-data/` 目錄產生出來。這一步必須做，否則下面要複製的目標目錄還不存在。

### 3. 強制停止 Taverncraft

進入系統設定 → 應用程式 → Taverncraft → **強制停止**，確保 Taverncraft 後台服務完全退出。遷移期間不要讓 Taverncraft 在後台執行，否則它可能在你複製檔案的同時改寫資料。

### 4. 把資料搬進 Taverncraft 資料目錄
...
Taverncraft APK 自己在 `luker-data/` 下維護一個 `_runtime-persist/` 目錄，存放執行時持久化產物。**不要覆蓋也不要刪除它**，從原版 SillyTavern 也不會帶來這個目錄。
:::

### 5. 重啟 Taverncraft，驗證資料

重新打開 Taverncraft APK。等待載入完成後角色卡列表、聊天紀錄、世界書、預設、API 連線都應自動還原。
...
| 內容 | Termux 原路徑 | Taverncraft APK 目標路徑 |
| --- | --- | --- |
...
引導只在全新資料目錄上首次開啟時彈出。如果 Taverncraft 之前已經被開啟過，從使用者管理面板的「備份與還原」做同樣的匯入即可——點「全選」，選 ZIP，選「增量更新」，然後點「還原備份」。
...
**重啟 Taverncraft 後角色卡列表是空的（手動複製流程）**

檢查複製層級：`luker-data/` 下應直接出現 `default-user/`（或其他使用者名稱目錄），而不是嵌套的 `luker-data/data/default-user/`。如果出現了重複的 `data/` 一層，把裡面的內容上移一級。

**Taverncraft 啟動後白屏 / 報錯（手動複製流程）**

可能是 `_runtime-persist/` 被覆蓋或破壞。可以嘗試：① 刪除 `_runtime-persist/` 讓 Taverncraft 自動重建；② 仍有問題時進入「應用程式資訊 → 儲存空間 → 清除資料」並重新走一遍遷移流程。
...
遷移後 Taverncraft 不會修改原 SillyTavern 資料格式，只在 `data/` 中追加自己的狀態檔案（如 `.luker-state.<chat_id>.json` 等）。如果你之後又想回到 Termux 上的 SillyTavern，把 `luker-data/` 的內容拷回去即可，SillyTavern 會忽略它無法辨識的狀態檔案。
...
- [快速開始](/zh-TW/guide/getting-started) — Taverncraft 安裝總覽
