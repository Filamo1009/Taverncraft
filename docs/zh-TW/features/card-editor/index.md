...
角色卡編輯助手（Character Editor Assistant，簡稱 CEA）是 Taverncraft 內建的 AI 輔助編輯工具。它讓你可以用自然語言指令修改角色卡設定、世界書條目和 CardApp 程式碼，每次 AI 做出的修改都會以差異對比的形式展示，由你逐項審批後才會生效——確保角色卡始終在你的掌控之中。
