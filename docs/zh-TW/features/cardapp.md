...
CardApp 是 Taverncraft 獨有的角色卡內嵌應用系統。它允許角色卡在 `data.extensions.card_app` 中定義小型應用（包含 HTML、JavaScript 和樣式），這些應用會在聊天介面中載入和渲染，為角色卡賦予互動式的動態能力。
...
2. **執行** — 應用透過 `ctx` 上下文物件與 Taverncraft 互動，回應聊天事件並更新自身狀態
